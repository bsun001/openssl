/*
    To compile:
    gcc server3.c common.c -o server3 -lcrypto -lssl -lpthread

    This code uses OpenSSL; it uses an SSL certificate, and validate it.
 */
#include "common.h"
...
SSL_CTX *setup_server_ctx(void)
{
    ...
    /* must present the intermediate certificate */
    if (SSL_CTX_use_certificate_chain_file(ctx, CERTFILE2) != 1)
        int_error("Error loading certificate from file");
    if (SSL_CTX_use_PrivateKey_file(ctx, CERTFILE2, SSL_FILETYPE_PEM) != 1)
        int_error("Error loading private key from file");
